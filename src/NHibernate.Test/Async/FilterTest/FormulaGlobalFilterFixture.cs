...
{
	[TestFixture]
	public class FormulaGlobalFilterFixtureAsync : TestCase
	{
		...
		public async Task TestCachingAsync()
		{
			...
			using (var tx = s.BeginTransaction())
			{
				...
				var orderStatus = await (s.CreateQuery("from OrderStatus s fetch all properties where s.Code = :code")
				                   .SetParameter("code", "SAV")
				                   .UniqueResultAsync<OrderStatus>());

				var statuses = await (s.Query<OrderStatus>()
